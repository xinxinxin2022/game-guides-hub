...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return generateSEO({
    title: `Palworld ${t('server')} Guide`,
    description: 'Complete Palworld server setup guide.',
    url: '/server-setup',
  });
}

export default async function ServerSetupPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = getT(locale, 'home.categories');
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="mb-12">
        <h1 className="text-4xl font-bold text-white">{t('server')} Setup Guides</h1>
        <p className="mt-4 text-lg text-zinc-400">Everything about Palworld server setup.</p>
      </div>
